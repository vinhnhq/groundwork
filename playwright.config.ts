...
export default defineConfig({
  ...
  webServer: {
    ...
    env: {
      ...
      // Sign-in is no longer mocked: it reads the `user` table better-auth
      // owns, so the suite needs the migrated + seeded database. Run
      // `bun run migrate && bun run seed` before `bun run test:e2e`.
