...
- ✎ **RV1a** Retire the capture/status-flip UI — decide the gap first. → **[P]**
  ...
  - **Touches:** `src/app/ops/[project]/actions.ts` · `src/app/ops/[project]/tasks/page.tsx` (the
    ...
    **Must NOT:** the tasks _read_ view — the table/board stay.
...
- [x] **F0.1** Scaffold Next 16 + pinned stack + toolchain. _(2026-07-24)_
- [x] **F1.1** Core lib seam — result/pipe/clock/repository/context + tests. _(db.ts/db-pool.ts runtime singletons deferred to F5; 2026-07-24)_
- [x] **F2.1** ContentSource: `project.yml` parser + filesystem adapter. _(2026-07-25)_
- [x] **F2.2** Markdown → RSC renderer (react-markdown + mermaid + relative-image rewrite). _(shipped in F4; 2026-07-25)_
- [x] **F3.1** Task model + DoR deriver + backlog parser. _(2026-07-25)_
- [x] **F4.1** `/ops` overview — project list + READY queue + DRAFT list. _(2026-07-25)_
- [x] **F4.2** Project detail + doc render pages + asset route. _(2026-07-25)_
- [x] **F6.1** Dogfood (Groundwork ingests itself) + ADR-0001 + status. _(2026-07-25)_
...
- [x] **G2** Paste door — "Copy context" button + `context.md` export route. → **[S]** _(2026-07-28)_
...
- [x] **G3** Local MCP server (stdio, read-only) — `list_projects` · `ready_tasks` · `get_project_context` · `get_doc`. → **[P]** _(2026-07-28)_
...
- [x] **G4** Remote MCP (HTTP + token) — the team's shared grounding endpoint. → **[P]** _(2026-07-28)_
...
- [x] **S1** Backlog serializer + the `BacklogWriter` seam. → **[P]** _(2026-07-28)_
...
- [x] **S2** Local-git write-back — commit on a `groundwork/*` branch. → **[P]** _(2026-07-28)_ _(ADR-0002)_
...
- [x] **S3** GitHub ContentSource (read) + signed push webhook. → **[P]** _(2026-07-28)_ _(ADR-0001 amendment)_
...
- [x] **S4** GitHub write-back — open a PR. → **[P]** _(2026-07-28)_ _(ADR-0002)_
...
- [x] **S5** Task capture + status flip UI (spec US-3/US-4). → **[S]** _(2026-07-28)_
...
- [x] **F5** Auth seam + better-auth over Kysely/Postgres. → **[P]** _(2026-07-29)_
...
### Q8 · Quality follow-ups — from the PR #8 QA pass _(2026-08-06)_
...
### Q · Quality follow-ups — from the PR #1 review _(2026-07-29)_
...
### W · Workspace UX — lift the infinite-oneness surfaces _(2026-07-29)_
...
> already byte-identical (ADR-0009); what is missing is the _composition_ — item rows, tabs, a
...
> `biz` digest no longer carries decisions, intents or oracles, but a task _title_ can still be
...
- [x] **W2** Docs as a real folder tree over the whole `__project__/`. → **[P]**
  - **Intent:** the source only scans three fixed paths (`docs/decisions/*`, `specs/*`,
    ...
    the _authoritative sources_ — are **not ingested at all**. The console cannot show the docs it
...
- [x] **W6** Grounding: relayout + audience-scoped digests. → **[P]**
  - **Intent:** one digest serves an engineer's agent and a client's agent equally badly — the first
    ...
    the team's internal reasoning. Splitting it is also a _disclosure_ control, not just ergonomics.
...
### v5 · Packaging — open-core _(DRAFT / stretch — deliberately not started)_
