...
their own agents — GPT/Claude — and *cannot* use git). Two problems fall out of that reality,
...
- **Task *editing model* beyond backlog.md** — no comments DB, no attachments store in v2 (repo
...
- **Write-back safety.** Default to **PR, not direct commit to main** (AI proposes / humans
  dispose). Token scoped to *Contents+PR: write* on listed repos only.
...
- **ADR-0005** Triage agent authority — read-only tools, proposes drafts only, human confirms
  write-back. Descendant of infinite-oneness ADR-0029. *(v3, US-6)*
- **ADR-0006** MCP surface & transport — tool contract; stdio (local) vs HTTP+token (remote/team);
  read-tools-first, no mutation tools in v2. *(v2, G3/G4)*
...
- **ADR-0007** Team pivot & delivery model — reverses the solo non-goal; open-core (self-host lib +
  optional Cloud), roles, BYOK/subscription cost model. The framing decision. *(records this spec)*
